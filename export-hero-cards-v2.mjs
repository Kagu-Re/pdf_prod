...
const exportHeroCardsV2 = async () => {
  console.log('🚀 Starting Hero Cards V2 PDF export...');
  ...
  try {
    ...
    const mergedPdfPath = path.join(outputDir, `digital-transformation-hero-cards-v2-${timestamp}.pdf`);
    fs.writeFileSync(mergedPdfPath, mergedPdfBytes);
    
    console.log(`✅ Hero Cards V2 PDF saved: ${mergedPdfPath}`);

    // Also save individual card files for reference
    console.log('📚 Creating individual card files for reference...');
    for (let i = 0; i < pdfBuffers.length; i++) {
      const cardPath = path.join(outputDir, `hero-v2-${cards[i].name}.pdf`);
      fs.writeFileSync(cardPath, pdfBuffers[i]);
      console.log(`  📄 Card ${i + 1}: ${cardPath}`);
    }

    // Create a comprehensive summary
    const summaryPath = path.join(outputDir, 'hero-cards-v2-summary.md');
    const summaryContent = `# Digital Transformation Hero Cards V2 PDF Export

## Export Details
- **Export Date**: ${new Date().toISOString()}
- **Component**: WaterRippleTriptych (Hero Cards V2)
...
## Files Generated
- **digital-transformation-hero-cards-v2-${timestamp}.pdf** - Main merged PDF file
- **hero-v2-hero-card.pdf** - Hero card with title and subtitle
- **hero-v2-card-01-problem.pdf** - Individual content cards with unique footers
- **hero-v2-card-02-discovery.pdf**
- **hero-v2-card-03-solution.pdf**
- **hero-v2-card-04-implementation.pdf**
- **hero-v2-card-05-future.pdf**
...
`;
    ...
    console.log('🎉 Hero Cards V2 PDF export completed successfully!');
...
// Run the export
exportHeroCardsV2().catch(console.error);
